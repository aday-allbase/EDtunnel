 ...
 */
let userID = 'fb7fa339-4d4c-419e-8ee7-cbcd5c0eb196';
...
function getConfig(userIDs, hostName) {
	const commonUrlPart = `?encryption=none&security=tls&sni=${hostName}&fp=randomized&type=ws&host=${hostName}&path=%2Fakun#${hostName}`;
...
function GenSub(userID_path, hostname) {
	...
	const randomPath = () => '/' + Math.random().toString(36).substring(2, 15) + 'akun';
	const commonUrlPartHttp = `?encryption=none&security=none&fp=random&type=ws&host=${hostname}&path=${encodeURIComponent(randomPath())}#`;
	const commonUrlPartHttps = `?encryption=none&security=tls&sni=${hostname}&fp=random&type=ws&host=${hostname}&path=%2Fakun#`;
